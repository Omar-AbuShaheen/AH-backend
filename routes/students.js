...
router.put('/profile', authenticateToken, requireStudent, async (req, res) => {
  try {
    const student_id = req.user.id;
    const { first_name, last_name, major, university, graduation_year, phone, skills, experience, location, bio } = req.body;
    
    const result = await db.query(
      'UPDATE student_profiles SET first_name = $1, last_name = $2, major = $3, university = $4, graduation_year = $5, phone = $6, skills = $7, experience = $8, location = $9, bio = $10, updated_at = CURRENT_TIMESTAMP WHERE user_id = $11 RETURNING *',
      [first_name, last_name, major, university, graduation_year, phone, skills || '', experience || '', location || '', bio || '', student_id]
    );
